jomo.lm.edited <- function (formula, data, beta.start = NULL, l1cov.start = NULL, 
                            ...
                            output = 1, out.iter = 10) 
{
  ...
  stopifnot(grepl("~", deparse(formula, width.cutoff = 200L)))
  ...
  for (j in 1:length.sub) {
    for (k in 1:order.sub[j]) {
      ...
      if (length(which(colnames(Y.con) == current.term)) == 
          0) {
  ...
  if (is.null(beta.start)) 
    beta.start = matrix(0, ncol(X), (max(0, ncol(Y.con)) + 
                                       max(0, (sum(Y.numcat) - length(Y.numcat)))))
  ...
  stopifnot(nrow(l1cov.start) == ncol(l1cov.start), nrow(l1cov.start) == 
              ncol(beta.start))
  stopifnot(nrow(l1cov.prior) == ncol(l1cov.prior), nrow(l1cov.prior) == 
              nrow(l1cov.start))
  ...
  imp = matrix(0, nrow(Y) * (nimp + 1), ncol(Y) + ncol(X) + 
                 3)
  imp[1:nrow(Y), 1] = Ysub
  ...
  imp[(nrow(X) + 1):(2 * nrow(X)), (ncol(Y) + 2):(ncol(Y) + 
                                                    ncol(X) + 1)] = X
  ...
  .Call("jomolm", Ysub, Ysubimp, submod, order.sub, Y, Yimp, 
        Yimp2, Y.cat, X, betaY.start, bYpost, betait, bpost, 
        varY.start, vYpost, covit, opost, nburn, varY.prior, 
        l1cov.prior, Y.numcat, ncolYcon, out.iter, PACKAGE = "jomo")
  bpost <- matrix(0, nrow(beta.start), ncol(beta.start))
  bYpost <- matrix(0, 1, length(betaY.start))
  opost <- matrix(0, nrow(l1cov.start), ncol(l1cov.start))
  vYpost <- matrix(0, 1, 1)
  ...
  if (!is.null(Y.con)) {
    imp[(nrow(Y) + 1):(2 * nrow(Y)), 2:(ncol(Y.con) + 1)] = Yimp2[, 
                                                                  1:ncol(Y.con)]
  }
  if (isnullcat == 0) {
    imp[(nrow(Y) + 1):(2 * nrow(Y)), (ncolYcon + 2):(ncol(Y) + 
                                                       1)] = Y.cat
  }
  if (output == 1) 
  ...
  for (i in 2:nimp) {
    ...
    imp[(i * nrow(X) + 1):((i + 1) * nrow(X)), (ncol(Y) + 
                                                  ncol(X) + 3)] = i
    .Call("jomolm", Ysub, Ysubimp, submod, order.sub, Y, 
          Yimp, Yimp2, Y.cat, X, betaY.start, bYpost, betait, 
          bpost, varY.start, vYpost, covit, opost, nbetween, 
          varY.prior, l1cov.prior, Y.numcat, ncolYcon, out.iter)
    betapost[, , (i - 1)] = bpost
  ...
  if (isnullcat == 0) {
    for (i in 1:ncol(Y.cat)) {
      imp[, (ncolYcon + 1 + i)] <- as.factor(imp[, (ncolYcon + 
                                                      1 + i)])
  ...
  if (is.null(colnamx)) 
    colnamx = paste("X", 1:ncol(X), sep = "")
  if (is.null(colnamysub)) 
    colnamysub = "Ysub"
  colnames(imp) <- c(colnamysub, colnamycon, colnamycat, colnamx, 
                     "id", "Imputation")
  return(imp)
}

jomo.glm.edited <- function (formula, data, beta.start = NULL, l1cov.start = NULL, 
                             l1cov.prior = NULL, nburn = 1000, nbetween = 1000, nimp = 5, 
                             output = 1, out.iter = 10, family = "binomial") 
{
  if (nimp < 2) {
    nimp = 2
    cat("Minimum number of imputations:2. For single imputation use function jomo.glm.MCMCchain\n")
  }
  if (family != "gaussian" & family != "binomial") 
    cat("ERROR: choose either family binomial or gaussian\n")
  if (family == "gaussian") {
    jomo.lm(formula, data, beta.start = beta.start, l1cov.start = l1cov.start, 
  ...
  if (family == "binomial") {
    stopifnot(is.data.frame(data))
    stopifnot(grepl("~", deparse(formula, width.cutoff = 200L)))
    fit.cr <- glm(formula, data = data, family = binomial, 
                  na.action = na.omit)
    betaY.start <- coef(fit.cr)
    varY.start <- 1
    varY.prior <- 1
    ...
    imp[(nrow(X) + 1):(2 * nrow(X)), (ncol(Y) + 2):(ncol(Y) + 
                                                      ncol(X) + 1)] = X
    imp[(nrow(X) + 1):(2 * nrow(X)), (ncol(Y) + ncol(X) + 
                                        2)] = c(1:nrow(Y))
    imp[(nrow(X) + 1):(2 * nrow(X)), (ncol(Y) + ncol(X) + 
                                        3)] = 1
    betapost <- array(0, dim = c(nrow(beta.start), ncol(beta.start), 
                                 (nimp - 1)))
    betaYpost <- array(0, dim = c(1, length(betaY.start), 
                                  (nimp - 1)))
    bpost <- matrix(0, nrow(beta.start), ncol(beta.start))
    bYpost <- matrix(0, 1, length(betaY.start))
    omegapost <- array(0, dim = c(nrow(l1cov.start), ncol(l1cov.start), 
    ...
    for (i in 1:length(Ysubimp)) if (is.na(Ysubimp[i])) 
      Ysubimp[i] = mean(Ysubimp, na.rm = TRUE)
    Ysubcat <- c(Ysub)
    .Call("jomoglm", Ysub, Ysubimp, Ysubcat, submod, order.sub, 
          Y, Yimp, Yimp2, Y.cat, X, betaY.start, bYpost, betait, 
          bpost, varY.start, vYpost, covit, opost, nburn, varY.prior, 
          l1cov.prior, Y.numcat, ncolYcon, out.iter, PACKAGE = "jomo")
    bpost <- matrix(0, nrow(beta.start), ncol(beta.start))
    bYpost <- matrix(0, 1, length(betaY.start))
    opost <- matrix(0, nrow(l1cov.start), ncol(l1cov.start))
    ...
    if (!is.null(Y.con)) {
      imp[(nrow(Y) + 1):(2 * nrow(Y)), 2:(ncol(Y.con) + 
                                            1)] = Yimp2[, 1:ncol(Y.con)]
    }
    if (isnullcat == 0) {
      imp[(nrow(Y) + 1):(2 * nrow(Y)), (ncolYcon + 2):(ncol(Y) + 
                                                         1)] = Y.cat
    ...
    for (i in 2:nimp) {
      ...
      imp[(i * nrow(X) + 1):((i + 1) * nrow(X)), (ncol(Y) + 
                                                    ncol(X) + 3)] = i
      .Call("jomoglm", Ysub, Ysubimp, Ysubcat, submod, 
            order.sub, Y, Yimp, Yimp2, Y.cat, X, betaY.start, 
            bYpost, betait, bpost, varY.start, vYpost, covit, 
            opost, nbetween, varY.prior, l1cov.prior, Y.numcat, 
    ...
    if (isnullcat == 0) {
      for (i in 1:ncol(Y.cat)) {
        imp[, (ncolYcon + 1 + i)] <- as.factor(imp[, 
                                                   (ncolYcon + 1 + i)])
        levels(imp[, (ncolYcon + 1 + i)]) <- previous_levels[[i]]
      }
    }
    ...
    if (is.null(colnamysub)) 
      colnamysub = "Ysub"
    colnames(imp) <- c(colnamysub, colnamycon, colnamycat, 
                       colnamx, "id", "Imputation")
    return(imp)
  }
}