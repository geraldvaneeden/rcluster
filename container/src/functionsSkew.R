...
#under consideration, is more than 1.5 sd's away from the mean.
MAR <- function(db, prob) {
  ...
  for(i in ncol(x):ncol(x)) {
    ...
    sd = sqrt(var(x[,2]))
    mar   = y
    y.mar = ifelse((mar<(mean-1.5*sd)) & (mcar<prop.m), NA, y)
    x[,i] <- y.mar
  }
  for(i in 2:(ncol(x)-1)) {
    y = db2[,i+1]
...
MNAR <- function(db, prob) {
  ...
  for(i in 2:ncol(x)) {
    ...
    sd = sqrt(var(x[,i]))
    mnar   = y
    y.mnar = ifelse((mnar<(mean-1.5*sd)) & (mcar<prop.m), NA, y)
    x[,i] <- y.mnar
  }
  return(x)
}
...
genMixedData <- function(baseData, dep_var_discr, no_of_discr_vars) {
  ...
  if(dep_var_discr == T){
    ...
    binomvar <- qbinom(1-pvars, 1, 0.61)
    dep_var <- binomvar
  }
  else {
    dep_var <- baseData[,1]
